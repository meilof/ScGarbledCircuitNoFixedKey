...
FastGarblingFourToTwoNoAssumptions::FastGarblingFourToTwoNoAssumptions(void)
{
}
FastGarblingFourToTwoNoAssumptions::~FastGarblingFourToTwoNoAssumptions(void)
{
...
FastGarblingFourToTwoNoAssumptions::FastGarblingFourToTwoNoAssumptions(const char* fileName)
{
...
void FastGarblingFourToTwoNoAssumptions::garble(block *emptyBothInputKeys, block *emptyBothOutputKeys, unsigned char *emptyTranslationTable, block seed){
	...
	for (int i = 0; i < numberOfGates; i++){
		...
		if (garbledGates[i].truthTable == XOR_GATE && garbledGates[i].input1 == -1){//This is a NOT gate
			...
			if (garbledGates[i].truthTable == XOR_GATE){
				...
				//encrypt 4ks_senc
				intrin_sequential_ks4_enc4((const unsigned char*)plaintext, (unsigned char*)ciphertext, 1, (unsigned char*)KEYS, (unsigned char*)keys, NULL);
				...
				*((unsigned char *)(&deltaOutput)) |= 1;
				...
				intrin_sequential_ks4_enc8((const unsigned char*)plainText, (unsigned char*)cipherText, 4, (unsigned char*)KEYS, (unsigned char*)keys, NULL);
...
void FastGarblingFourToTwoNoAssumptions::initAesEncryptionsAndInputKeys(block* emptyBothInputKeys){
...
void  FastGarblingFourToTwoNoAssumptions::compute(block * singleWiresInputKeys, block * Output)
{
...
bool FastGarblingFourToTwoNoAssumptions::internalVerify(block *bothInputKeys, block *emptyBothWireOutputKeys){
